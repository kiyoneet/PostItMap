...
class AccountController extends Controller
{
    ...
    public function registerAction()
    {
        ...
        } else if (!preg_match('/^\w{3,20}$/', $email_address)) {
            $errors[] = 'ユーザIDは半角英数字およびアンダースコアを3 ～ 20 文字以内で入力してください';
        } else if (!$this->db_manager->get('User')->isUniqueUserName($email_address)) {
        ...
        if (count($errors) === 0) {
            $this->db_manager->get('User')->insert($email_address, $password);
            $this->session->setAuthenticated(true);

            $user = $this->db_manager->get('User')->fetchByUserName($email_address);
    ...
    public function authenticateAction()
    {
        ...
        if (count($errors) === 0) {
            ...
            $user = $user_repository->fetchByUserName($email_address);
